...
async.map = function (/* hlog, */ arr, method, resultsHandler)
{
	...
	function checkDone ()
	{
		...
		{
			args.resultsHandler.call(context, context.currentLog, results);
...
{
	this.currentLog = httpLog(hlog);
}

AsyncContext.prototype.log = function (hlog)
{
	this.currentLog = httpLog.chain(this.currentLog, hlog);
};
	...
	function createCallback (iterator)
	{
		...
		var cb = function ()
		{
			...
			process.nextTick(function ()
			{
				...
				if (!iterator.fn || _break || context.currentLog.failed)
				{
					args[0] = context.currentLog || httpLog.none;
