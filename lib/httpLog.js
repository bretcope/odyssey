...
// The minimum failure code
var MIN_FAIL_CODE = 400;
...
[
	...
	{ name: 'response', instance: Http.IncomingMessage, mutex: ['error', 'message'] },
	{ name: 'message', type: 'string', mutex: ['error', 'response'] },
...
function HttpLog ()
{
	var args = arguer.apply(_constructorFormat, arguments);
	if (args instanceof Error)
		return httpLog(args);

	if (args.response instanceof Http.IncomingMessage && args.response.statusCode)
		return httpLog(args.response.statusCode, Http.STATUS_CODES[args.response.statusCode]);
...
Object.defineProperty(HttpLog.prototype, '__gettingStack',
{
	value: false,
	writable: true
});

Object.defineProperty(HttpLog.prototype, '__previous',
{
	value: null,
	writable: true
});
...
Object.defineProperty(HttpLog.prototype, 'failed',
{
	get: function ()
	{
		return this.highestLevel >= MIN_FAIL_CODE;
	}
});
...
HttpLog.prototype.toString = function ()
{
	...
	do
	{
		var stack = getStack(log);
		if (log.status >= MIN_FAIL_CODE && stack)
		{
			result += stack;
		}
		else if (log.message)
		...
		{
			result += Error.prototype.toString.call(this);
		}
...
{
	log.__gettingStack = true;
	var stack = log.stack;
	log.__gettingStack = false;
	return stack;
}
